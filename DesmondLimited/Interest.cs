using System;
using System.Collections.Generic;
namespace DesmondLimited
{
    public class Interest
    {
        public IList<Customer> Customers {get; set; } = new List<Customer>();

        public Interest(IList<Customer> customers)
        {
            Customers = customers;
        }

        

        public IList<decimal> SimpleInterest()
        {
            IList<decimal> simpInt = new List<decimal>();
            foreach (var customer in Customers)
            {
                decimal interest = customer.Principal * (decimal)customer.Rate * (decimal)customer.Time;
                simpInt.Add(interest);
            }
            return simpInt;
        }
        public IList<decimal> CompdInterest()
        {
            IList<decimal> compdInterests = new List<decimal>();
            foreach (var customer in Customers)
            {
                decimal interest = customer.Principal * (decimal)(Math.Pow((1.0 + customer.Rate), (double)customer.Time));
                compdInterests.Add(interest);
            }
            return compdInterests;
        }

    }
}